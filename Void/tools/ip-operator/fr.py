...
def afficher(ip, data):
    ...
    table = Table(
        title=f"[bold red]IP Opérateur[/bold red] — [bold white]{ip}[/bold white]",
        box=box.DOUBLE_EDGE,
        border_style="red",
    ...
    table.add_row("Nom AS",          data.get("asname", "N/A"))
...
if __name__ == "__main__":
    try:
        console.print()
        console.print(Panel(
            "[bold red]VOID[/bold red] — [bold white]IP Opérateur[/bold white]",
        ...
        with Progress(
            ...
            p.add_task("Récupération opérateur...", total=None)
